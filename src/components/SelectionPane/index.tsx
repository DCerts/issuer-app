...
const SelectionPane = (props: SelectionPaneProps) => {
    ...
    const search = (text?: string) => {
        if (text == undefined || text.trim() === '') {
            ...
            setOptions(props.options.filter(option => {
                return selectedOptions.includes(option)
                    || option.trim().toLowerCase().includes(text.trim().toLowerCase());
            }));
        }
    };
                        ...
                        <WaitingForTransaction>
                            ...
                            <div className={styles.options}>
                                {options.map((option, index) => (
                                    <div
                                        className={
                                            classNames(styles.option, { [styles.selected]: selectedOptions.includes(option) })
                                        }
                                        onClick={() => {
                                            const currentSelectedOptions = [...selectedOptions];
                                            const foundIndex = currentSelectedOptions.indexOf(option);
                                            if (foundIndex != -1) {
                                                currentSelectedOptions.splice(foundIndex, 1);
